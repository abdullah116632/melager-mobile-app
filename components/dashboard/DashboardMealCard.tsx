...
const MenuMarquee = ({ text }: { text: string }) => {
  ...
  return (
    <View
      className="ml-1 h-[17px] min-w-0 flex-1 justify-center overflow-hidden"
    ...
    >
      ...
      >
        ...
        <View className="mt-1 flex-row items-center">
          <Text
            className={`font-inter-semibold text-[12px] ${statusColor}`}
            numberOfLines={1}
          >
            {status} ·
          </Text>
      ...
      <Pressable
        className="h-11 w-[46px] items-center justify-center"
      ...
      >
        {pending ? (
          ...
          <View
            pointerEvents="none"
            className={`h-[22px] w-[40px] justify-center rounded-full p-[2px] ${isOn ? "bg-teal-200" : "bg-slate-300"}`}
            style={{ alignItems: isOn ? "flex-end" : "flex-start" }}
          >
            <View
              className={`h-[18px] w-[18px] rounded-full ${isOn ? "bg-teal-600" : "bg-slate-500"}`}
              style={{
                shadowColor: "#0F172A",
      ...
      <View className="ml-2 min-w-[66px] items-center rounded-md bg-teal-50 px-2 py-1.5">
        <Text
          className="font-inter-bold text-[13px] leading-[15px] text-teal-700"
          style={{ opacity: enabled ? 1 : 0.42 }}
          numberOfLines={1}
        >
          {activeCount}
        ...
        <Text
          className="font-inter-semibold text-[8px] leading-[10px] text-teal-700"
          style={{ opacity: enabled ? 1 : 0.42 }}
          numberOfLines={1}
        >
          Active meals
