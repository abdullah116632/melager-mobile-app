...
interface DepositConsumerDetailModalProps {
...
export const DepositConsumerDetailModal = ({
  ...
  totalDeposits,
  onClose,
}: DepositConsumerDetailModalProps) => (
  <Modal
  ...
  >
    <View className="flex-1 justify-end bg-black/45">
      <TouchableOpacity
        ...
        activeOpacity={1}
        onPress={onClose}
        accessibilityLabel="Close deposit consumer details"
      />
      <View className="rounded-t-3xl bg-white px-5 pb-6 pt-3">
        ...
        <View className="mt-4 rounded-2xl border border-teal-200 bg-teal-50 px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <Text
              className={`font-inter-bold text-[22px] ${totalDeposits < 0 ? "text-red-600" : "text-teal-700"}`}
              numberOfLines={1}
              adjustsFontSizeToFit
              minimumFontScale={0.75}
            >
              {"\u09F3"}
              {formatDepositAmount(totalDeposits)}
            </Text>
          </View>
        </View>
